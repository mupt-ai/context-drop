...
export interface RuntimeServerOptions { now?: () => Date; afterTaskPersisted?: (runId: string) => void; recovery?:Partial<RuntimeRecoveryPolicy> }
...
function prepareTask(config: RuntimeConfig, owner: {routerId:string;chatId:string}, task: string, authorization: Authorization | undefined, now: Date): { id:string; request:LaunchRequest; record:TaskRecord } {
  ...
  const record:TaskRecord = { id:`task_${id}`, runId:id, ...owner, task, reportCapability, createdAt, updatedAt:createdAt, status:"launching", authorizationId:authorization?.id, authorizedAction:authorization?.action, authorizedScope:authorization?.scope, authorizationExpiresAt:authorization?.expiresAt };
...
function requireActiveSlot(config:RuntimeConfig,owner:{routerId:string;chatId:string}):void { const active=records<TaskRecord>(pathFor(config,"parent-tasks.jsonl")).filter(t=>t.status==="launching"||t.status==="running"); if(active.length>=MAX_ACTIVE_TASKS_GLOBAL)throw new Error("delegated worker capacity reached"); if(active.filter(t=>t.routerId===owner.routerId&&t.chatId===owner.chatId).length>=MAX_ACTIVE_TASKS_PER_OWNER)throw new Error("delegated worker capacity reached for this chat"); }
...
function reconcileAndCompact(config:RuntimeConfig,current:Date,policy:RuntimeRecoveryPolicy):void {
  ...
  for(const task of tasks){
    ...
    if(task.status==="launching"&&age>=policy.launchTimeoutMs){task.status="launch_failed";task.launchError="launch did not complete before recovery timeout";task.reportCapability="";task.updatedAt=current.toISOString();}
  ...
  const activeRunIds=new Set(tasks.filter(t=>t.status==="launching"||t.status==="running").map(t=>t.runId));
  ...
  const activeTasks=tasks.filter(t=>t.status==="launching"||t.status==="running"||protectedRuns.has(t.runId));
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/v1/delegate") {
        const owner = routerFor(config, auth(req)); if (!owner) return json(res, 401, { error: "unauthorized" }); const input = await body(req); if (typeof input?.task !== "string" || !input.task.trim() || Buffer.byteLength(input.task) > 16000) throw new Error("task is required and must be <= 16000 bytes"); requireActiveSlot(config,owner);
        const prepared = prepareTask(config, owner, input.task, undefined, current); const taskPath = pathFor(config, "parent-tasks.jsonl"); append(taskPath, prepared.record); options.afterTaskPersisted?.(prepared.id);
        try { const run = prepared.request.backend === "herdr" ? launchInHerdr(config, prepared.request, prepared.id, runner) : launchInTmux(config, prepared.request, prepared.id, runner); const tasks=records<TaskRecord>(taskPath); const persisted=tasks.find(t=>t.runId===prepared.id)!; persisted.status="running"; persisted.updatedAt=current.toISOString(); replace(taskPath,tasks); append(pathFor(config,"runs.jsonl"),run); return json(res,201,{taskId:prepared.record.id,run}); }
        catch(err) { const tasks=records<TaskRecord>(taskPath); const persisted=tasks.find(t=>t.runId===prepared.id); if(persisted){persisted.status="launch_failed"; persisted.launchError=err instanceof Error?err.message:"launch failed"; persisted.reportCapability=""; persisted.updatedAt=current.toISOString(); replace(taskPath,tasks);} throw err; }
      }
      if (req.method === "POST" && url.pathname === "/v1/confirm") {
        ...
        requireActiveSlot(config,owner); const tasks = records<TaskRecord>(pathFor(config, "parent-tasks.jsonl")); const original = tasks.find(t => t.runId === challenge.runId); if (!original) throw new Error("challenge task is missing"); const authorization:Authorization={id:`auth_${randomBytes(24).toString("base64url")}`,action:challenge.sensitiveAction!,scope:challenge.challengedAction!,expiresAt:challenge.challengeExpiresAt}; const reservationId=randomBytes(16).toString("base64url"); challenge.challengeReservationId=reservationId; challenge.challengeReservationUntil=new Date(current.getTime()+policy.reservationMs).toISOString(); challenge.authorizationId=authorization.id; replace(reportPath,reports);
        const prepared=prepareTask(config,owner,original.task,authorization,current); const taskPath=pathFor(config,"parent-tasks.jsonl"); append(taskPath,prepared.record); options.afterTaskPersisted?.(prepared.id); try { const run=prepared.request.backend==="herdr"?launchInHerdr(config,prepared.request,prepared.id,runner):launchInTmux(config,prepared.request,prepared.id,runner); const persisted=records<TaskRecord>(taskPath); const launched=persisted.find(t=>t.runId===prepared.id)!; launched.status="running"; launched.updatedAt=current.toISOString(); replace(taskPath,persisted); append(pathFor(config,"runs.jsonl"),run); const finalReports=records<ParentReport>(reportPath); const finalChallenge=finalReports.find(r=>r.id===challenge.id&&r.challengeReservationId===reservationId); if(!finalChallenge) throw new Error("confirmation reservation was lost"); finalChallenge.challengeConsumedAt=current.toISOString(); delete finalChallenge.challengeReservationId; delete finalChallenge.challengeReservationUntil; replace(reportPath,finalReports); return json(res,201,{run,action:authorization.action,scope:authorization.scope,expiresAt:authorization.expiresAt}); } catch(err){ const persisted=records<TaskRecord>(taskPath); const task=persisted.find(t=>t.runId===prepared.id); if(task&&task.status==="launching"){task.status="launch_failed";task.launchError=err instanceof Error?err.message:"launch failed";task.reportCapability="";task.updatedAt=current.toISOString();replace(taskPath,persisted);} const retryReports=records<ParentReport>(reportPath); const retryChallenge=retryReports.find(r=>r.id===challenge.id&&r.challengeReservationId===reservationId&&!r.challengeConsumedAt); if(retryChallenge){delete retryChallenge.challengeReservationId;delete retryChallenge.challengeReservationUntil;delete retryChallenge.authorizationId;replace(reportPath,retryReports);} throw err; }
      }
      if (req.method === "POST" && url.pathname === "/v1/reports") {
        const input = await body(req); if (typeof input?.runId !== "string" || !REPORT_KINDS.has(input.kind) || typeof input.message !== "string" || !input.message.trim() || input.message.length > 4000) throw new Error("invalid report");
        ...
        const tasks = records<TaskRecord>(pathFor(config, "parent-tasks.jsonl")); const task = tasks.find(t => t.runId === input.runId); if (!task || !capMatches(auth(req), task.reportCapability)) return json(res, 401, { error: "unauthorized" }); const all = records<ParentReport>(pathFor(config, "parent-reports.jsonl")); if (all.some(r => r.runId === task.runId && (r.kind === "completed" || r.kind === "failed"))) return json(res, 409, { error: "task already terminal" }); const cutoff = current.getTime() - 3600_000; if (all.filter(r => r.runId === task.runId && Date.parse(r.createdAt) >= cutoff).length >= REPORT_LIMIT_PER_HOUR) return json(res, 429, { error: "report rate limit exceeded" });
        const report: ParentReport = { id: `report_${current.getTime().toString(36)}_${randomBytes(5).toString("hex")}`, runId: task.runId, routerId: task.routerId, chatId: task.chatId, kind: input.kind, message: input.message, sensitiveAction, challengedAction, challengeToken: sensitiveAction ? randomBytes(6).toString("hex").toUpperCase() : undefined, challengeExpiresAt:sensitiveAction?new Date(current.getTime()+CHALLENGE_MS).toISOString():undefined, createdAt: current.toISOString() }; append(pathFor(config, "parent-reports.jsonl"), report); task.updatedAt=current.toISOString(); if (input.kind === "completed" || input.kind === "failed") task.status = input.kind; replace(pathFor(config, "parent-tasks.jsonl"), tasks); return json(res, 201, { report });
      }
