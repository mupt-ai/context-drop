...
For an actionable trusted-chat architecture, pass `--router-mode`. The router is structurally started with built-in tools, ambient extensions, and skills disabled; its only execution tool is the explicit `delegate(task)` extension. The runtime launches each delegated worker visibly in the configured Herdr/tmux backend, using a private `<CONTEXT_DROP_HOME>/delegation/workers` cwd rather than a repo path supplied by the router. Workers receive a per-run capability for `report_to_parent`; the general runtime token is never put in the Pi environment or prompt. Reports are bounded, persisted in private runtime state, and atomically claimed before concise iMessage delivery. Claiming is intentionally at-most-once: a daemon crash after claim can lose a notification, but cannot duplicate it after restart. Active-run follow-ups use a safe new-worker continuation fallback because the runtime has no supported steering API; no terminal keystrokes are synthesized. Payment, password/MFA recovery, and materially changed terms remain explicit confirmation gates.
