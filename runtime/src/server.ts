...
function requireOwnedHerdrPane(config: RuntimeConfig, owner: RouterCapability, paneId: string): RunRecord {
  const match=ownedHerdrRuns(config,owner).find(({run})=>run.herdrPane===paneId&&run.status!=="exited");
  if(!match)throw new Error("Herdr pane is not owned by this conversation");
  return match.run;
}
function ownedHerdrTopology(config: RuntimeConfig, owner: RouterCapability, runner: CommandRunner) {
  const topology=herdrTopology(config,runner),owned=ownedHerdrRuns(config,owner),paneIDs=new Set(owned.map(item=>item.run.herdrPane).filter((id):id is string=>Boolean(id))),tabIDs=new Set(owned.map(item=>item.run.herdrTab).filter((id):id is string=>Boolean(id))),workspaceIDs=new Set(owned.map(item=>item.run.herdrWorkspace).filter((id):id is string=>Boolean(id)));
  return {...topology,workspaces:topology.workspaces.filter(item=>workspaceIDs.has(item.workspaceId)),tabs:topology.tabs.filter(item=>tabIDs.has(item.tabId)),panes:topology.panes.filter(item=>paneIDs.has(item.paneId)),agents:topology.agents.filter(item=>paneIDs.has(item.paneId))};
}
  ...
  const server = createServer((req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" && url.pathname === "/v1/tasks") {
        const owner = routerFor(config, auth(req)); if (!owner) return json(res, 401, { error: "unauthorized" }); const live = liveTaskStatus(config, runner); observeManagedLiveTasks(config, current, runner, live); const saved = records<TaskRecord>(pathFor(config, "parent-tasks.jsonl")); const runs = loadRuns(config); const allowedRuns=new Map(runs.flatMap(run=>{const record=saved.find(item=>item.runId===run.id&&item.chatId===owner.chatId&&(item.routerId===owner.routerId||item.routerId===SCHEDULE_ROUTER_ID));const pane=run.backend==="herdr"?run.herdrPane:run.tmuxPane;const rightSession=run.backend!=="herdr"||run.herdrSession===(config.herdrSession||"default");return record&&pane&&rightSession?[[pane,{run,record}] as const]:[];})); const tasks=live.tasks.filter(task=>allowedRuns.has(task.paneId));for(const task of tasks){const owned=allowedRuns.get(task.paneId)!;task.fullyManaged=true;task.name=owned.record.label??task.name;} let topology; if (live.backend === "herdr") { try { topology = ownedHerdrTopology(config,owner,runner); } catch { /* legacy Herdr/mocks may expose only agent list */ } } return json(res, 200, { tasks, ...(topology ? { topology } : {}) });
      }
      if (req.method === "GET" && url.pathname === "/v1/herdr/overview") {
        const owner=routerFor(config, auth(req));if(!owner)return json(res, 401, { error: "unauthorized" }); return json(res, 200, { topology: ownedHerdrTopology(config,owner,runner) });
      }
      ...
      if (req.method === "POST" && url.pathname === "/v1/herdr/read") {
        const owner=routerFor(config, auth(req));if(!owner)return json(res, 401, { error: "unauthorized" }); const input = await body(req); if (typeof input?.paneId !== "string" || (input.lines !== undefined && typeof input.lines !== "number")) throw new Error("paneId and numeric lines are required");requireOwnedHerdrPane(config,owner,input.paneId); return json(res, 200, { paneId: input.paneId, output: readHerdrAgent(config, input.paneId, input.lines ?? 120, runner) });
      }
      if (req.method === "POST" && url.pathname === "/v1/herdr/status") {
        const owner=routerFor(config, auth(req));if(!owner)return json(res, 401, { error: "unauthorized" }); const input=await body(req);if(typeof input?.paneId!=="string")throw new Error("paneId is required");requireOwnedHerdrPane(config,owner,input.paneId);return json(res,200,herdrAgentStatus(config,input.paneId,runner));
      }
