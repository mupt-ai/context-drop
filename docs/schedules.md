...
A scheduled worker that uses `context-drop report` should send exactly one concise final user-facing message. Context Drop coalesces unsent reports from the same schedule run and waits for the worker to finish before delivery, preventing start/progress chatter from leaking into chat. If a scheduled Pi worker omits the report, Context Drop can recover its authoritative settled assistant text when it is suitable for delivery; otherwise the job fails with a visible lifecycle notice instead of remaining pending. A workflow that sends directly with another messaging tool must not also report the same result; that external delivery is outside Context Drop's report receipt tracking.
