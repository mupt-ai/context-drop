...
test("failed authorized launch releases challenge for exactly one successful retry", async () => {
  const c={...config(),defaultBackend:"herdr" as const};let starts=0;const flaky:CommandRunner={run(command,args){if(command==="herdr"&&args.includes("workspace"))return{status:0,stdout:JSON.stringify({result:{workspace:{workspace_id:"w"},tab:{tab_id:"t"},root_pane:{pane_id:"p"}}})};if(args.includes("pane")&&args.includes("run")&&++starts===2)return{status:1,stderr:"injected launch failure"};return{status:0};}};
