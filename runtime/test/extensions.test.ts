...
test("faithful loader executes router and report extensions over real HTTP", async () => {
  ...
  try {
    ...
    const report=await loadExtension(join(runtimeRoot,"dist","src","report_to_parent_extension.js"),dir); report.default({registerTool:(t:any)=>tool=t}); const result=await tool.execute("2",{kind:"needs_user",message:"confirm",sensitiveAction:"payment_or_purchase"},new AbortController().signal); assert.ok(result.details.report.challengeToken); assert.equal(result.details.report.chatId,"chat-a");
