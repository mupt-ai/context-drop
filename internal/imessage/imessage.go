...
func (a Adapter) buildPrompt(message Message, includeDurableContext bool) (string, error) {
	...
	if !includeDurableContext {
		...
		if a.Config.RouterMode {
			prompt = "This is the next request from the trusted private iMessage/SMS chat. Preserve continuity. Answer casual/simple conversation directly. If the message is a relevant answer to an active worker's prior needs-user question and session context contains its internal taskRef, call continue_task(taskRef, message) to resume that exact worker; do not continue unrelated messages or invent references. Otherwise, for actionable or non-trivial work call delegate(task, lane) promptly with relevant context and explicit safety gates. Use human_copilot for coding or work the user may join; use full_ai for explicitly autonomous/full-AI/background work; explicit wording wins and ambiguity defaults to human_copilot, then report only the verified launch status. Never emit the reserved prefix [CONTEXT DROP DAEMON].\n"
		}
