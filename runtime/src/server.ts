...
function prepareTask(config: RuntimeConfig, owner: {routerId:string;chatId:string}, task: string, lane: DelegationLane, authorization: Authorization | undefined, now: Date): { id:string; request:LaunchRequest; record:TaskRecord } {
  ...
  const request: LaunchRequest = { agent, repo: workerCwd(config), prompt: workerPrompt(task, authorization), name: `task-${id.slice(-10)}`, backend: config.defaultBackend, lane, extension: reportExtension(), environment };
  const createdAt=now.toISOString();
  const record:TaskRecord = { id:`task_${id}`, runId:id, ...owner, task, lane, reportCapability, createdAt, updatedAt:createdAt, status:"launching", authorizationId:authorization?.id, authorizedAction:authorization?.action, authorizedScope:authorization?.scope, authorizationExpiresAt:authorization?.expiresAt };
  ...
  const server = createServer((req, res) => {
    ...
    try {
      ...
      if (req.method === "POST" && url.pathname === "/v1/delegate") {
        const owner = routerFor(config, auth(req)); if (!owner) return json(res, 401, { error: "unauthorized" }); const input = await body(req); if (typeof input?.task !== "string" || !input.task.trim() || Buffer.byteLength(input.task) > 16000) throw new Error("task is required and must be <= 16000 bytes"); const lane:DelegationLane=input.lane??"human_copilot"; if(lane!=="human_copilot"&&lane!=="full_ai")throw new Error("lane must be human_copilot or full_ai"); requireActiveSlot(config,owner);
        const prepared = prepareTask(config, owner, input.task, lane, undefined, current); const taskPath = pathFor(config, "parent-tasks.jsonl"); append(taskPath, prepared.record); options.afterTaskPersisted?.(prepared.id);
        let externalStarted=false; try { const run = prepared.request.backend === "herdr" ? launchInHerdr(config, prepared.request, prepared.id, runner) : launchInTmux(config, prepared.request, prepared.id, runner); externalStarted=true; const tasks=records<TaskRecord>(taskPath); const persisted=tasks.find(t=>t.runId===prepared.id)!; persisted.status="running"; persisted.updatedAt=current.toISOString(); replace(taskPath,tasks); append(pathFor(config,"runs.jsonl"),run); return json(res,201,{taskId:prepared.record.id,run}); }
