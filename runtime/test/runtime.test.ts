...
const runner: CommandRunner = { run(command, args) {
  if (command === "herdr" && args[2] === "workspace" && args[3] === "list") return { status: 0, stdout: JSON.stringify({ result: { workspaces: [{ workspace_id: "managed", label: "ContextDropManaged" }] } }) };
...
test("delegation lanes share one Herdr session and ambiguous defaults to human copilot", async () => {
  const c={...config(),defaultBackend:"herdr" as const}; const calls:string[][]=[]; let managed=false; const recording:CommandRunner={run(command,args){if(command==="herdr")calls.push(args);if(args[2]==="workspace"&&args[3]==="list")return{status:0,stdout:JSON.stringify({result:{workspaces:managed?[{workspace_id:"managed",label:"ContextDropManaged"}]:[]}})};if(args[2]==="workspace"&&args[3]==="create"){managed=true;return{status:0,stdout:JSON.stringify({result:{workspace:{workspace_id:args.includes("ContextDropManaged")?"managed":"human"},tab:{tab_id:"first-tab"},root_pane:{pane_id:"first-pane"}}})}};if(args[2]==="tab"&&args[3]==="create")return{status:0,stdout:JSON.stringify({result:{tab:{tab_id:"managed-tab"},root_pane:{pane_id:"managed-pane"}}})};return{status:0};}};
  ...
  try{const cap=await issue(base,headers);const implicit=await delegate(base,cap,"ambiguous work");const autonomous=await delegate(base,cap,"explicit autonomous work","full_ai");assert.equal(implicit.run.lane,"human_copilot");assert.equal(implicit.run.herdrSession,"default");assert.equal(implicit.run.herdrWorkspace,"human");assert.equal(autonomous.run.lane,"full_ai");assert.equal(autonomous.run.herdrSession,"default");assert.equal(autonomous.run.herdrWorkspace,"managed");assert.ok(calls.every(args=>args[1]==="default"));const tasks=readFileSync(join(c.stateDir,"parent-tasks.jsonl"),"utf8").trim().split("\n").map(line=>JSON.parse(line));assert.deepEqual(tasks.map(t=>t.lane),["human_copilot","full_ai"]);const invalid=await fetch(base+"/v1/delegate",{method:"POST",headers:{authorization:`Bearer ${cap}`,"content-type":"application/json"},body:JSON.stringify({task:"bad",lane:"spoofed in task"})});assert.equal(invalid.status,400);}
  finally{await close(server);}
});

test("full-AI creates one managed workspace then reuses it with new tabs", () => {
  const c=config();const calls:string[][]=[];let managed=false,tab=0;const recording:CommandRunner={run(_command,args){calls.push(args);if(args[2]==="workspace"&&args[3]==="list")return{status:0,stdout:JSON.stringify({result:{workspaces:managed?[{workspace_id:"opaque-managed",label:"ContextDropManaged"}]:[]}})};if(args[2]==="workspace"&&args[3]==="create"){managed=true;return{status:0,stdout:JSON.stringify({result:{workspace:{workspace_id:"opaque-managed"},tab:{tab_id:"managed:t1"},root_pane:{pane_id:"managed:p1"}}})}};if(args[2]==="tab"&&args[3]==="create"){tab++;return{status:0,stdout:JSON.stringify({result:{tab:{tab_id:`managed:t${tab+1}`},root_pane:{pane_id:`managed:p${tab+1}`}}})}};return{status:0};}};
...
test("explicit workspaceId wins over the managed full-AI workspace", () => {
  const c=config();const calls:string[][]=[];const explicit:CommandRunner={run(_command,args){calls.push(args);if(args[2]==="tab"&&args[3]==="create")return{status:0,stdout:JSON.stringify({result:{tab:{tab_id:"explicit:t1"},root_pane:{pane_id:"explicit:p1"}}})};return{status:0};}};const run=launchInHerdr(c,{agent:"mock",repo:c.stateDir,prompt:"work",lane:"full_ai",workspaceId:"explicit-w"},"run_explicit",explicit);assert.equal(run.herdrWorkspace,"explicit-w");assert.equal(run.herdrTab,"explicit:t1");assert.ok(calls.every(args=>args[2]!=="workspace"||args[3]!=="list"));
});

test("full-AI resolves a concurrent managed-workspace creation using opaque IDs", () => {
  const c=config();let lists=0;const calls:string[][]=[];const racing:CommandRunner={run(_command,args){calls.push(args);if(args[2]==="workspace"&&args[3]==="list"){lists++;return{status:0,stdout:JSON.stringify({result:{workspaces:lists===1?[]:lists===2?[{workspace_id:"winner",label:"ContextDropManaged"},{workspace_id:"ours",label:"ContextDropManaged"}]:[{workspace_id:"winner",label:"ContextDropManaged"}]}})}};if(args[2]==="workspace"&&args[3]==="create")return{status:0,stdout:JSON.stringify({result:{workspace:{workspace_id:"ours"},tab:{tab_id:"ours:t1"},root_pane:{pane_id:"ours:p1"}}})};if(args[2]==="workspace"&&args[3]==="close")return{status:0};if(args[2]==="tab"&&args[3]==="create")return{status:0,stdout:JSON.stringify({result:{tab:{tab_id:"winner:t2"},root_pane:{pane_id:"winner:p2"}}})};return{status:0};}};const run=launchInHerdr(c,{agent:"mock",repo:c.stateDir,prompt:"work",lane:"full_ai"},"run_race",racing);assert.equal(run.herdrWorkspace,"winner");assert.equal(run.herdrTab,"winner:t2");assert.ok(calls.some(args=>args.slice(2,5).join(" ")==="workspace close ours"));
});

test("full-AI fails safely when managed workspace discovery is unavailable, malformed, or ambiguous", () => {
  for(const [name,response,pattern] of [["unavailable",{status:1,stderr:"server unavailable"},/discovery failed/],["invalid",{status:0,stdout:"not json"},/invalid JSON/],["malformed",{status:0,stdout:JSON.stringify({result:{workspaces:[{workspace_id:"w1"}]}})},/malformed/],["ambiguous",{status:0,stdout:JSON.stringify({result:{workspaces:[{workspace_id:"w1",label:"ContextDropManaged"},{workspace_id:"w2",label:"ContextDropManaged"}]}})},/ambiguous/]] as const){assert.throws(()=>launchInHerdr(config(),{agent:"mock",repo:config().stateDir,prompt:"work",lane:"full_ai"},`run_${name}`,{run(_command,args){if(args[2]==="workspace"&&args[3]==="list")return response;throw new Error("unexpected launch after failed discovery");}}),pattern);}
});
...
test("ambiguous delegated launcher failure immediately revokes reporting", async () => {
  const c={...config(),defaultBackend:"herdr" as const}; const ambiguous:CommandRunner={run(command,args){if(args[2]==="workspace"&&args[3]==="list")return{status:0,stdout:JSON.stringify({result:{workspaces:[]}})};if(command==="herdr"&&args.includes("workspace"))return{status:0,stdout:JSON.stringify({result:{workspace:{workspace_id:"w"},tab:{tab_id:"t"},root_pane:{pane_id:"p"}}})};if(args.includes("pane")&&args.includes("run"))return{status:1,stderr:"connection lost after dispatch"};return{status:0};}}; const server=createRuntimeServer(c,"secret",ambiguous);await new Promise<void>(r=>server.listen(0,"127.0.0.1",r));const address=server.address();assert.ok(address&&typeof address==="object");const base=`http://127.0.0.1:${address.port}`,headers={authorization:"Bearer secret","content-type":"application/json"};
