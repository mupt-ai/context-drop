...
test("extensions execute over real HTTP", async () => {
  ...
  try {
    ...
    const listed=await tools.get("list_tasks").execute("list",{},new AbortController().signal);assert.equal(listed.details.tasks.length,1);assert.equal(listed.details.tasks[0].label,"Research launch behavior");assert.equal(listed.details.tasks[0].backend,"herdr");assert.ok(listed.details.tasks[0].taskRef);
    const bumped=await tools.get("bump_task").execute("bump",{taskRef:listed.details.tasks[0].taskRef,message:"please post a fresh status"},new AbortController().signal);assert.equal(bumped.details.bumped,true);assert.equal(bumped.details.task.label,"Research launch behavior");
