...
| `CONTEXT_DROP_HERDR_SESSION` | Herdr session used by delegated workers (default `default`). |
| `CONTEXT_DROP_FULL_AI_HERDR_WORKSPACE_LABEL` | Reusable full-AI workspace label (default `ContextDropManaged`). |
...
Runtime credentials from environment variables are not written back to the config file by unrelated commands. Runtime token/config, daemon state, schedules, and logs live under the same private Context Drop root with user-only permissions. The runtime config records a `defaultBackend` (`tmux` or `herdr`), a `herdrSession` for human-copilot workers (default `default`; full-AI always uses session `default`), and a reusable `fullAIHerdrWorkspaceLabel` (default `ContextDropManaged`); `herdrPath` is resolved at initialization so the daemon can run Herdr even with a minimal service-manager PATH. Initialization resolves and persists an absolute `nodePath` in the runtime config so the OS service can start Node even when its environment has a minimal `PATH`; re-run `context-drop init` if Node moves.
