...
function liveHerdrTasks(config: RuntimeConfig, runner: CommandRunner): LiveTaskStatus[] {
  ...
  if (agents.some(agent => !agent || typeof agent.agent !== "string" || !agent.agent || typeof agent.agent_status !== "string" || !agent.agent_status || typeof agent.pane_id !== "string" || !agent.pane_id || (agent.focused !== undefined && typeof agent.focused !== "boolean"))) {
...
function liveTmuxTasks(config: RuntimeConfig, runner: CommandRunner): LiveTaskStatus[] {
  const separator = "\u001f";
  const format = ["#{pane_id}", "#{window_name}", "#{pane_current_command}", "#{pane_dead}", "#{pane_active}"].join(separator);
  ...
  return (listed.stdout ?? "").split("\n").filter(Boolean).map(line => {
    const [paneId, window, command, dead, active] = line.split(separator);
    if (!paneId?.startsWith("%") || !window || command === undefined || (dead !== "0" && dead !== "1") || (active !== "0" && active !== "1")) throw new Error("live tmux status returned malformed pane data");
    return { paneId, name: window, agent: command || "unknown", status: dead === "1" ? "exited" : "running", selected: active === "1", fullyManaged: false };
  });
}
