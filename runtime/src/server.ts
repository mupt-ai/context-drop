...
function supersedeAuthorizedSource(config:RuntimeConfig,sourceRunId:string,replacementRunId:string,current:Date,runner:CommandRunner):void {
  try {
    ...
    try { cleanupConfirmed=run?.backend==="herdr"?closeHerdrWorker(config,run,runner):run?.backend==="tmux"?closeTmuxWorker(config,run,runner):false; } catch { /* replacement is committed; cleanup is best effort */ }
    if(!cleanupConfirmed){
  ...
  const server = createServer((req, res) => {
    ...
    try {
      ...
      if (req.method === "POST" && url.pathname === "/v1/tasks/delegate") {
        const owner = routerFor(config, auth(req)); if (!owner) return json(res, 401, { error: "unauthorized" }); const input = await body(req); if (typeof input?.prompt !== "string" || !input.prompt.trim() || Buffer.byteLength(input.prompt) > 16000) throw new Error("prompt is required and must be <= 16000 bytes"); const agent = input.agent === undefined ? validateDelegateAgent(config) : input.agent; if (typeof agent !== "string" || !config.agents[agent]) throw new Error("configured agent is required"); const name = input.name === undefined ? undefined : input.name; if (name !== undefined && (typeof name !== "string" || !name.trim() || Buffer.byteLength(name) > 120)) throw new Error("name must be <= 120 bytes"); const label = name?.trim() || `${agent} task`; requireActiveSlot(config, owner); const prepared = prepareTask(config, owner, input.prompt, label, "full_ai", undefined, current, agent); prepared.request.name = label; const taskPath = pathFor(config, "parent-tasks.jsonl"); append(taskPath, prepared.record); options.afterTaskPersisted?.(prepared.id); let externalStarted = false; try { const run = prepared.request.backend === "herdr" ? launchInHerdr(config, prepared.request, prepared.id, runner) : launchInTmux(config, prepared.request, prepared.id, runner); externalStarted = true; const tasks = records<TaskRecord>(taskPath); const persisted = tasks.find(t => t.runId === prepared.id)!; persisted.status = "running"; persisted.updatedAt = current.toISOString(); replace(taskPath, tasks); append(pathFor(config, "runs.jsonl"), run); const paneId = run.backend === "herdr" ? run.herdrPane : run.tmuxPane; return json(res, 201, { task: { paneId, agent: run.agent, name: label, status: "running", selected: false, fullyManaged: true } }); } catch (err) { const tasks = records<TaskRecord>(taskPath); const persisted = tasks.find(t => t.runId === prepared.id); if (persisted) { persisted.status = externalStarted || err instanceof LaunchOutcomeUnknownError ? "launch_unknown" : "launch_failed"; persisted.launchError = err instanceof Error ? err.message : "launch failed"; persisted.reportCapability = ""; persisted.updatedAt = current.toISOString(); replace(taskPath, tasks); } throw err; }
      }

      if (req.method === "GET" && url.pathname === "/v1/tasks") {
        const owner = routerFor(config, auth(req)); if (!owner) return json(res, 401, { error: "unauthorized" }); const live = liveTaskStatus(config, runner); observeManagedLiveTasks(config, current, runner, live); const saved = records<TaskRecord>(pathFor(config, "parent-tasks.jsonl")); const runs = loadRuns(config); const managed = new Map(runs.map(run => [run.backend === "herdr" ? run.herdrPane : run.tmuxPane, run])); for (const task of live.tasks) { const run = managed.get(task.paneId); const record = run && saved.find(item => item.runId === run.id && item.routerId === owner.routerId && item.chatId === owner.chatId); if (record) { task.fullyManaged = true; task.name = record.label ?? task.name; } } return json(res, 200, { tasks: live.tasks });
      }
      if (req.method === "POST" && url.pathname === "/v1/tasks/continue") {
        const owner = routerFor(config, auth(req)); if (!owner) return json(res, 401, { error: "unauthorized" }); const input = await body(req); if (typeof input?.paneId !== "string" || typeof input?.prompt !== "string" || !input.prompt.trim() || Buffer.byteLength(input.prompt) > 16000) throw new Error("paneId and prompt are required; prompt must be <= 16000 bytes"); const live = liveTaskStatus(config, runner).tasks.find(task => task.paneId === input.paneId); if (!live) return json(res, 404, { error: "live task pane not found" }); const managedRun = loadRuns(config).find(run => (run.backend === "herdr" ? run.herdrPane : run.tmuxPane) === input.paneId); if (managedRun) { const managedTask = records<TaskRecord>(pathFor(config, "parent-tasks.jsonl")).find(task => task.runId === managedRun.id); if (managedTask?.authorizationId) throw new Error("authorized sensitive workers cannot be continued; request a fresh exact authorization"); } const prompt = input.prompt.trim(); if (config.defaultBackend === "herdr") continueLiveHerdr(config, config.herdrSession || "default", input.paneId, prompt, runner); else continueLiveTmux(config, input.paneId, prompt, runner); return json(res, 200, { task: live, continued: true });
      }
