...
test("human copilot creates a meaningfully named no-focus workspace when none is focused", () => {
  const c=config(),calls:string[][]=[];const recording:CommandRunner={run(_command,args){calls.push(args);if(args[2]==="workspace"&&args[3]==="list")return{status:0,stdout:JSON.stringify({result:{workspaces:[{workspace_id:"w1",label:"Existing",focused:false}]}})};if(args[2]==="workspace"&&args[3]==="create")return{status:0,stdout:JSON.stringify({result:{workspace:{workspace_id:"copilot"},tab:{tab_id:"copilot:t1"},root_pane:{pane_id:"copilot:p1"}}})};return{status:0};}};
  const run=launchInHerdr(c,{agent:"mock",repo:c.stateDir,prompt:"work",lane:"human_copilot"},"run_fallback",recording);assert.equal(run.herdrWorkspace,"copilot");const create=calls.find(args=>args[2]==="workspace"&&args[3]==="create")!;assert.deepEqual(create.slice(4),["--cwd",c.stateDir,"--label","Context Drop Copilot","--no-focus"]);
});
...
test("explicit workspaceId wins over lane placement and uses a privacy-safe tab label", () => {
  const c=config();const calls:string[][]=[];const explicit:CommandRunner={run(_command,args){calls.push(args);if(args[2]==="tab"&&args[3]==="create")return{status:0,stdout:JSON.stringify({result:{tab:{tab_id:"explicit:t1"},root_pane:{pane_id:"explicit:p1"}}})};return{status:0};}};const run=launchInHerdr(c,{agent:"mock",repo:c.stateDir,prompt:"credential https://secret.test",name:"task-random",lane:"full_ai",workspaceId:"explicit-w"},"run_explicit",explicit);assert.equal(run.herdrWorkspace,"explicit-w");assert.equal(run.herdrTab,"explicit:t1");assert.ok(calls.every(args=>args[2]!=="workspace"||args[3]!=="list"));const create=calls.find(args=>args[2]==="tab"&&args[3]==="create")!;assert.ok(create.includes("Context Drop AI"));assert.ok(!create.join(" ").includes("credential"));assert.ok(!create.join(" ").includes("task-random"));
});
