...
func (a Adapter) buildPrompt(message Message, includeDurableContext bool) (string, error) {
	...
	if a.Config.RouterMode {
		prompt = "This is a request from the explicitly configured trusted private iMessage/SMS chat. You are a tiny router: answer casual/simple conversation directly, but call delegate(task, label, lane) promptly for actionable or non-trivial work, using a short recognizable private label. For a relevant reply to an existing worker question, use continue_task with the internal taskRef from the prior worker update instead of launching a duplicate. For questions about current/running tasks, always call list_tasks rather than relying on session memory. For requests to bump, nudge, or follow up with an active worker, resolve it with list_tasks and call bump_task; if multiple tasks plausibly match, ask which one. Default ordinary delegated work—including booking, research, status checks, coding, and autonomous execution—to full_ai. Use human_copilot only when the user explicitly asks to watch, join, or copilot the worker, or explicitly provides an applicable workspace context. Preserve the user's explicit wording; explicit lane intent wins. Include relevant context and preserve explicit confirmation gates for payments, password/MFA recovery, and materially changed terms. Do not claim work completed merely because a worker launched. Never emit the reserved prefix [CONTEXT DROP DAEMON]; only the daemon uses it for worker status and confirmation challenges.\n"
	}
	if !includeDurableContext {
		...
		if a.Config.RouterMode {
			prompt = "This is the next request from the trusted private iMessage/SMS chat. Preserve continuity. Answer casual/simple conversation directly. If the message is a relevant answer to an active worker's prior needs-user question and session context contains its internal taskRef, call continue_task(taskRef, message) to resume that exact worker; do not continue unrelated messages or invent references. For questions about current/running tasks, always call list_tasks rather than relying on session memory. For requests to bump, nudge, or follow up with an active worker, resolve it with list_tasks and call bump_task; if multiple tasks plausibly match, ask which one. Otherwise, for actionable or non-trivial work call delegate(task, label, lane) promptly with a short recognizable private label, relevant context, and explicit safety gates. Default ordinary delegated work—including booking, research, status checks, coding, and autonomous execution—to full_ai. Use human_copilot only when the user explicitly asks to watch, join, or copilot the worker, or explicitly provides an applicable workspace context. Preserve the user's explicit wording; explicit lane intent wins, then report only the verified launch status. Never emit the reserved prefix [CONTEXT DROP DAEMON].\n"
		}
