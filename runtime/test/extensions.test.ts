...
test("router extension exposes exactly the three task tools and disables them for report turns", async () => {
  const runtimeRoot=dirname(dirname(dirname(fileURLToPath(import.meta.url)))),repoRoot=dirname(runtimeRoot),dir=mkdtempSync(join(tmpdir(),"cd-router-"));process.env.CONTEXT_DROP_DELEGATE_URL="http://127.0.0.1/v1/delegate";process.env.CONTEXT_DROP_DELEGATE_CAPABILITY="cap";const router=await loadExtension(join(repoRoot,"internal","imessage","pi_router_extension.mjs"),dir);let before:any,toolCall:any;const tools:string[]=[],active:string[][]=[];router.default({registerTool:(tool:any)=>tools.push(tool.name),on:(name:string,fn:any)=>{if(name==="before_agent_start")before=fn;if(name==="tool_call")toolCall=fn;},setActiveTools:(value:string[])=>active.push(value)});assert.deepEqual(tools,["list_tasks","delegate_task","continue_task"]);const options={selectedTools:["delegate_task"],toolSnippets:["delegate_task"],promptGuidelines:["delegate_task"]};before({prompt:"CONTEXT_DROP_INTERNAL_REPORT_SUMMARY_V1\nuntrusted",systemPrompt:"base",systemPromptOptions:options});assert.deepEqual(active,[[]]);assert.deepEqual(options,{selectedTools:[],toolSnippets:[],promptGuidelines:[]});assert.deepEqual(toolCall(),{block:true,reason:"tools are disabled for internal report summaries",terminate:true});before({prompt:"normal",systemPrompt:"base",systemPromptOptions:options});assert.deepEqual(active[1],["list_tasks","delegate_task","continue_task"]);
});
