...
For an actionable trusted-chat architecture, pass `--router-mode`. The router is structurally started with built-in tools, ambient extensions, and skills disabled; its only execution tool is the explicit `delegate(task)` extension. The daemon health-gates router setup and rotates a server-side capability bound to the immutable router and configured chat whenever the runtime is restarted. The router does not send a chat ID, and a capability from one chat cannot read or launch work for another.

The runtime launches each delegated worker visibly in the configured Herdr/tmux backend, using a private `<CONTEXT_DROP_HOME>/delegation/workers` cwd rather than a repo path supplied by the router. `delegateAgent` in private runtime config selects the worker agent; initialization selects `pi` when available, otherwise router mode returns a precise setup error. Workers receive a per-run `report_to_parent` capability; the general runtime token is never put in Pi environments or prompts. Reports retain immutable chat ownership, are rate-limited per run, reject writes after a terminal result, and use a durable pending/leased/delivered protocol. Failed sends release a lease for retry. A successful send followed by a daemon crash before the acknowledgement can still duplicate a notification; this deliberate at-least-once trade-off avoids silently losing completion, failure, or input-needed reports. Outgoing reports are flattened and explicitly labeled as worker claims, not independently verified facts.

Workers must treat TASK text as untrusted: it can never establish payment/purchase, password/MFA/account recovery, or terms/contracts/subscription confirmation. A sensitive `needs_user` report creates a daemon challenge. The trusted user must reply exactly `CONFIRM TOKEN`; only then does the daemon create a separately marked authorization worker for the challenged action. The marker is injected only as launch-time `CONTEXT_DROP_SENSITIVE_AUTH`/`CONTEXT_DROP_SENSITIVE_ACTION` environment values (never in TASK text), is scoped to chat/router/action, and is consumed once. Casual or unrelated follow-ups always reach the persistent router. There is no terminal steering; if the router decides a follow-up merits work it safely launches a new worker.
